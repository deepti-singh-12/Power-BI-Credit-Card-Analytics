...
a.	Income Group Filter: Allow filtering by income levels (Low, Medium, High). Created Column using DAX formula. Formula Used: Income_Group = 
SWITCH(TRUE(),
        customer[Income]<= 35000, "Low Income",
        customer[Income]< 70000, "Medium Income",
        customer[Income]>= 70000, "High Income")

b.	Age Group Filter: Filter customers based on age ranges (e.g., 18-25, 26-35, etc.). Created a column using DAX Formula. Formula Used: Age_Group = 
SWITCH(TRUE(),
